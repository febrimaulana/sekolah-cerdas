...
import { getDataPenugasan, addDataPenugasan, deleteDataPenugasan, updateDataPenugasan } from '../../../config/redux/action/SumberDayaManusia';
...
const Penugasan = () => {    
    ...
    const onCreate =  async (values, status) => {        
        ...
        if (status === 'tambah') {                                  
            await dispatch(addDataPenugasan(dataInput));
            dispatch(getDataPenugasan({pagination: pagination}));
        } else if (status === 'ubah') {            
            ...
            await dispatch(updateDataPenugasan(dataUbah));
            dispatch(getDataPenugasan({pagination: pagination}));
        }
    };
    ...
    const handleHapus = (record) => {         
        ModalConfirm(
            ...
            'Hapus',
            async () => {
                await dispatch(deleteDataPenugasan({idStudent: record.id_siswa}));
                dispatch(getDataPenugasan({pagination: pagination}));
            }
        )           
    }
    ...
    const dataTable = {
        ...
        dataRow: stateSumberDayaManusia.dataPenugasan.data,
        idRow: 'id_siswa',
    ...
    const dataPagination = {
        ...
        countData: stateSumberDayaManusia.dataPenugasan.total,
        ...
        <div className="animated fadeIn">
            <div className="card">
                <div className="card-header">
                    <i className="icon-user"></i> DAFTAR PENUGASAN {stateRoot.name}
                </div>
